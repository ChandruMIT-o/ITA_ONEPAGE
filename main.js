...
function onScroll() {
    ...
    sections.forEach((section, index) => {
        ...
        if (scrollOffset + viewportHeight / 2 >= sectionTop && scrollOffset + viewportHeight / 2 < sectionBottom) {
...
window.addEventListener('DOMContentLoaded', () => {
    const homeButton = document.querySelector('.menu-button[data-target="#section-1"]');
...
container.addEventListener('wheel', (event) => {
    ...
    let nextIndex = Math.min(Math.max(currentIndex + direction, 0), sections.length - 1);
